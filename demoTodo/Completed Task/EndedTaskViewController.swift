...
class EndedTaskViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {

    @IBOutlet weak var table: UITableView!

    let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
    var DBManager = [CompletedTask]()
    ...
    func fetchData() {
        do {
            self.DBManager = try context.fetch(CompletedTask.fetchRequest())
    ...
    func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let edit = UIContextualAction(style: .normal, title: "Edit", handler: { (context, view, actionPerformed: (Bool) -> ()) in
            ...
            self.performSegue(withIdentifier: "editCompletedViewController", sender: nil)
